...
d3.csv('/assets/fairytalesSample.csv').then(data => {
  ...
  var filterTales = new dc.CboxMenu("#filterTales")
	...
	dataSummaryTable.dimension(yearDim)
      ...
      .size(Infinity)				       // need all the records & let pagination handle display & offset
    	.columns(['Tale', 'Title', 'Author', 'Date', {label: 'Country', format: function (d) { return d.FirstPublicationLocation}}, 'Notes'])  // can change labels & format of data if desired
