...
d3.csv('/fairytales/assets/fairytales2Dummy.csv').then(data => {
    ...
    countrySearchDim = ndx.dimension(d => d.FirstPublicationLocation),
    ...
    countrySearchGroup = countrySearchDim.group().reduceCount(),
  ...
  var filterTales = new dc.CboxMenu("#filterTales")
      ...
      //.group(composerGroup)
      .dimension(countrySearchDim)     // new DIM - graph updates BUT loose all
      .group(countrySearchGroup)
      //.order(function (a,b) {
...
function updateTale() {
  ...
  if (composerDim.currentFilter()) {
    ...
    $("#this-tale").html(firstrow["Composer"]);
    // pull intro from a new data source (eg json/csv of title => intro)
    var introstring = "";
